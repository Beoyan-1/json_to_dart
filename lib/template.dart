...
/*
 * @Author: Beoyan
 * @Date: 2023-03-09 10:59:32
 * @LastEditTime: 2023-03-10 09:41:41
 * @LastEditors: Beoyan
 * @Description: 
 */
import 'dart:convert';

import 'json_generator.dart' as main;
...
class DefaultTemplate extends Template {
  ...
  DefaultTemplate({required this.srcJson, this.className = "Entity"});
...
class ListTemplate extends DefaultTemplate {
  ...
  ListTemplate(
      {required String srcJson,
      String className = "Entity",
      required this.delegateTemplate})
  ...
  @override
  String constructor() {
    return delegateTemplate.constructor();
  }

  @override
  String field() {
    return delegateTemplate.field();
  }

  @override
  String method() {
    return delegateTemplate.method();
  }

  @override
  String end() {
    return delegateTemplate.end();
  }
...
class ListField extends Field {
  ...
  String get typeName {
    ...
    if (item is List) {
      ...
      type = main.isCamelCase
          ? camelTypeCase(nameString)
          : firstLetterUpper(nameString);
    } else if (item is int) {
      type = "int";
...
String camelTypeCase(String name) {
  StringBuffer sb = StringBuffer();
  var list = name.split("_");
  for (int i = 0; i < list.length; i++) {
    var item = list[i];
